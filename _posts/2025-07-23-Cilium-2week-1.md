---
layout: post
title:  "[Cilium Study 1기 by Gasida] 2주차 - 도전과제1 "
...
위 명령으로 system이라는 이름의 로그 구성이 생성되고, 해당 필터에 맞는 Flow만 /var/run/cilium/hubble/events-system.log에 기록된다.
```bash
...
위에서 job이름을 hubble로 설정 하였으므로 {job="hubble"}을 넣어 쿼리해보자.
```yaml
- job_name: hubble-flowlogs
  static_configs:
  - targets:
