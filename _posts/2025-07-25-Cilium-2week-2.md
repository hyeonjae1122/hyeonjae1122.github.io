...
--set hubble.tls.auto.method=helm \
--set hubble.tls.auto.certValidityDuration=1095 \
--set hubble.metrics.tls.enabled=true # <===추가
```

`Hubble metrics API`에서 mTLS를 사용하여 인증을 활성화하려면 먼저 클라이언트 인증서를 확인하는데 사용할 CA 인증서가 있는
ConfigMap을 만든다
...
# 2. Self-signed CA 인증서 생성 (유효기간: 3년)
openssl req -x509 -new -nodes -key ca.key -sha256 -days 1095 -out ca.crt \
  -subj "/C=US/ST=Example/L=Example City/O=MyOrg/OU=Hubble/CN=Hubble CA"

kubectl -n kube-system create configmap hubble-metrics-ca --from-file=ca.crt
```


Helm 명령어에 다음 플래그를 추가하여 mTLS 활성화
```bash
helm upgrade cilium cilium/cilium --namespace kube-system --reuse-values \
--set hubble.enabled=true \
--set hubble.relay.enabled=true \
...
위의 구성이 적용된 후, 클라이언트는 `Hubble metric API`에 접근하기 위해서 CA 인증서로 서명된 인증서를 사용하여 인증해야한다. 
