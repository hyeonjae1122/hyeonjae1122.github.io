...
‘HAProxy 통계 페이지’에 backend 대상 서버 2대 UP 확인.
![[2026-01-24- k8s 컨트롤플레인 HA환경 구성 따라해보기-13.png]]
...
# k8s api 호출 확인 : 각 controlplane IP
...
MacOS 터미널에서 `vagrant ssh k8s-ctr1` 접속

```bash
cat /etc/kubernetes/admin.conf  # 메모(복사)
```
...
MacOS 터미널에서 `vagrant ssh admin-lb` 접속

```bash
mkdir ~/.kube
touch ~/.kube/config
vi ~/.kube/config   # k8s-ctr1 에서 복사한 내용을 아래 붙여넣기
...
kubelet -> k8s api 통신 시, k8s api 접속 엔드포인트 확인
