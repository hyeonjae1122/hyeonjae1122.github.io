...
- 백업대상 이름 메모 : `etcd-snapshot-k8s-node1-1771680423`
...
- 서비스 중단 :  모든 서버 노드에서 RKE2 서비스를 멈춤

```bash
systemctl stop rke2-server
```

- 첫번째 서버 노드에서만 해당 스냅샷 이름으로 복구명령을 실행한다
...
| 항목         | 내용                                                                       |
| ---------- | ------------------------------------------------------------------------ |
| 다중 서버 환경   | 복원은 **첫 번째 서버 노드에서만** 실행, 나머지는 이후 재시작                                    |
